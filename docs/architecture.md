...
identity owns users, password credentials, sessions, account lifecycle, generic role-based access control, and the human-challenge lifecycle contract. IDs are PostgreSQL `uuid` values generated as UUIDv7 by Go 1.27; PostgreSQL 18 schema checks also reject non-v7 IDs. Host applications own OAuth providers, SSH keys, concrete challenge providers, business permission vocabulary, audit events, and domain relationships. PostgreSQL 18+ is the supported production baseline.
...
- Human challenges are an extensible boundary: the host supplies `HumanChallengeProvider` (which may keep in-memory state for image challenges); identity owns the public configuration and challenge endpoints, optional login/registration enforcement, and the module methods used by protected host actions.
